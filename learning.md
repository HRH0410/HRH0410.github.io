...
### 2026-02-09（About + 项目页二次重构）
- 错误：About This Site 的引用区第一次改版装饰感过强、留白偏大，和页面其它模块节奏不一致。
  - 影响：你反馈“不协调”，需要重复调整。
  - 修正动作：改成更克制的居中引言条（弱化符号、缩短留白、统一蓝灰色系、单独优化 dark 模式）。
  - 防复发规则：在 About 页这类信息段，优先与全页节奏一致，再追求“艺术感”。

- 错误：引用区样式迭代时出现了重复 dark 伪元素规则，导致视觉结果不稳定。
  - 影响：同一区块在不同状态下呈现不一致，维护成本上升。
  - 修正动作：删除重复选择器，保留单一生效规则并重新构建验证。
  - 防复发规则：每次样式迭代后都检查同名选择器是否重复定义（尤其 `.dark` 分支）。

- 错误：项目页标题避让最初沿用文章页思路，未意识到 `/projects/` 是 section 页面，不走 `#single_header`。
  - 影响：`项目` 标题被 fixed nav 遮挡。
  - 修正动作：在 `content/projects/_index.md` 内增加页面专属避让规则：`main#main-content > article > header > h1`。
  - 防复发规则：先确认页面模板类型（single/list/section），再决定避让样式挂载点。

- 错误：项目页重写后，HTML 缩进触发 Markdown 代码块渲染（出现 `<pre><code>`）。
  - 影响：代表项目和课程作品区块结构错乱。
  - 修正动作：将内联 HTML 块改为顶格写法，消除缩进触发；重建后检查 `public/projects/index.html`。
  - 防复发规则：在 Markdown 文件中写原生 HTML 时，默认使用“无缩进块结构”。

- 新偏好：项目页希望“明显有个性/有艺术感”，但仍需与主站的克制高级风一致。
  - 触发条件：你要求“项目页大改”“太普通”。
  - 默认执行：先重构版式（结构创新）再做视觉层（纹理/层次/动效），避免只改颜色和圆角。

- 新偏好：引用文案视觉不宜过大，强调气质但不压正文。
  - 触发条件：About This Site 引用段落。
  - 默认执行：采用中等字号与留白，优先保证与正文比例协调。
